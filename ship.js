...
instructions_array.forEach((instruction) => {
	// Action N means to move north by the given value.
	if (instruction[0] === "N"){
		y += parseInt(instruction[1], 10)
	}
	...
	// Action L means to turn left the given number of degrees.
		...
		direction -= parseInt(instruction[1], 10)
	}
	...
	// Action F means to move forward by the given value in the direction the ship is currently facing.
		...
		if (direction % 360 === 0) {
			...
			x -= parseInt(instruction[1], 10)
		} else if (direction % 360 === 270 || direction === -90){
			y += parseInt(instruction[1], 10)
		}
	}
...
const manhattan_distance = Math.abs(x) +  Math.abs(y) 
console.log(x);
console.log(y);
console.log(manhattan_distance)
