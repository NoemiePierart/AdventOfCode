...
seats.forEach((seat) => {
	...
	for (let i = 0; i < 7; i++ ){
		if (seat[i] === "F" ){
			max_row = max_row - Math.round((max_row - min_row) / 2) 
		} else if (seat[i] === "B"){
			min_row = min_row + Math.round((max_row - min_row) / 2)
		}
		// console.log(seat)
		// console.log(min_row)
		// console.log(max_row)
	...
	for (let i = 7; i < 10; i ++){
		if (seat[i] === "L" ){
			max_col = max_col - Math.round((max_col - min_col) / 2) 
		} else if (seat[i] === "R"){
			min_col = min_col + Math.round((max_col - min_col) / 2)
		}
	...
	const seat_id = (min_row * 8) + min_col;
	found_seats_ids.push(seat_id)
	// console.log(`${seat} : ${min_row} - ${max_row} => ${seat_id}`)

})
// console.log(found_seats_ids);

let missing_tickets = [];

// Compare the two lists : 
for (let k = 0; k < overall_seats_ids.length; k ++){
	if (found_seats_ids.includes(overall_seats_ids[k]) === false) {
		missing_tickets.push(overall_seats_ids[k]);
	}
}

console.log(missing_tickets)